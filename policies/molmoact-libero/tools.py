"""MolmoAct LIBERO checkpoint as a closed-loop VLA-policy skill.
...
the bundle: ``preset = "molmoact-libero"`` resolves the MolmoAct LIBERO
...
both as the bundle's callable (the ``MolmoActLibero`` class) and as a flat
tool ``molmoact-libero.run`` for callers that invoke it as a single unit.
"""
...
class MolmoActLibero(PolicyLoopSkill):
    """MolmoAct LIBERO checkpoint (Franka Panda, LIBERO pick-and-place)."""

    preset = "molmoact-libero"

    meta = SkillMeta(
        description=(
            "Run the MolmoAct LIBERO checkpoint "
            "(allenai/MolmoAct-7B-D-LIBERO-0812) in closed loop for a Franka "
            "Panda pick-and-place segment. Reads the graph-scoped "
            "observation_stream every window; terminates on a gripper cycle, "
            "a VLM yes/no check, or max_windows. The policy server is the "
            "bundle's own preset (no policy_id)."
        ),
        params=dict(_PARAMS),
...
@tool(
    name="molmoact-libero.run",
    summary=(
        "Run the MolmoAct LIBERO checkpoint in closed loop "
